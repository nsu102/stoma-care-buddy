// triage.ts
// 문진(Triage) 알고리즘 & 처방 데이터베이스 (TypeScript 변환)
...
export const questions: Record<string, Question> = {
  E_Q1: { 
    id: "E_Q1", 
    type: "question", 
    text: "현재 피가 멈추지 않고 계속 흐르나요?", 
    options: ["예 (응급)", "아니요"] 
  },
  E_Q2: { 
    id: "E_Q2", 
    type: "question", 
    text: "24시간 이상 가스/대변 배출이 없고 복통/구토가 있나요?", 
    options: ["예", "아니요"] 
  },
  E_Q3: { 
    id: "E_Q3", 
    type: "question", 
    text: "장루 색깔이 검게/보라색으로 변했거나 창백해졌나요?", 
    options: ["예", "아니요"] 
  },
  E_Q4: { 
    id: "E_Q4", 
    type: "question", 
    text: "장루가 길게 튀어나와 아프거나 피부 사이가 벌어졌나요?", 
    options: ["예", "아니요"] 
  },
  ...
  C1_Q1: { 
    ...
    options: ["장루가 하얗게 변함", "피부만 하얗게 붊", "선홍색 정상"] 
  },
  C1_Q2: { 
    id: "C1_Q2", 
    type: "question", 
  ...
  C2_Q2_SUB: { 
    ...
    type: "question", 
    text: "통증은 어떤가요?", 
    options: ["욱신거림/매끈함", "스치면 극심한 통증"] 
  },
  C2_Q3: { 
    id: "C2_Q3", 
    type: "question", 
    text: "붉은 부위 질감/시기가 어떤가요?", 
    options: ["쭈글쭈글/축축함", "오래된 자국", "최근/따가움"] 
  },
  C2_Q4: { 
    id: "C2_Q4", 
    type: "question", 
  ...
  C3_Q2_SUB: { 
    ...
    type: "question", 
    text: "통증과 감각은 어떤가요?", 
    options: ["욱신거리는 멍", "스치면 극심함", "차가움/감각없음"] 
  },
  C3_Q3: { 
    id: "C3_Q3", 
    type: "question", 
...
export const TREATMENT_DATA: Record<string, TreatmentInfo> = {
  ...
  "기계적 손상 [L2]": { 
    ...
    advice: "[관리법] 상처 부위에 파우더를 뿌리고 보호 필름을 덧발라 보호막을 만들어주십시오. 제거제를 꼭 사용하십시오.", 
    emergency_alert: null 
  },
  "기계적 손상 [L3]": { 
  ...
  for (const [keyword, info] of Object.entries(TREATMENT_DATA)) {
    if (diagnosisText.includes(keyword)) {
      ...
      if (info.emergency_alert) {
        ...
        if (!isSafe) {
          maxRiskLevel = Math.max(maxRiskLevel, 1) as RiskLevel;
        }
      }
    }
  }
...
export function getNextStep(
  ...
  if (currentStepId === "E_Q4") {
    if (answerIndex === 0) return finalResult("감돈된 장루 탈출");
      ...
      if (aiClass === 1) return questions["C1_Q1"];
      else if (aiClass === 2) return questions["C2_Q1"];
      else if (aiClass === 3) return questions["C3_Q1"];
  ...
  if (currentStepId === "C2_Q4") {
    ...
    else if (answerIndex === 3) return toCommon("모낭염");
  ...
  if (currentStepId === "C2_Q5") {
    if (answerIndex === 0 || answerIndex === 1 || answerIndex === 2) {
      return {
        ...questions["C2_SACS"],
        temp_diagnosis: answerIndex === 0 ? "자극성 피부염" : answerIndex === 1 ? "기계적 손상" : "모낭염"
      };
    }
    else return toCommon("상세 불명의 발적");
  }

  if (currentStepId === "C2_SACS") {
    const baseType = savedDiagnosis || "자극성 피부염";
    const sacsLevel = answerIndex === 0 ? "[L1]" : answerIndex === 1 ? "[L2]" : "[L3]";
    return toCommon(`${baseType} ${sacsLevel}`);
  }
  ...
  if (currentStepId === "C3_Q1") {
    if (answerIndex === 0) return toCommon("정상");
    else if (answerIndex === 1) return toCommon("육아종");
  ...
  if (currentStepId === "C3_Q2") {
    if (answerIndex === 0) return questions["C3_Q2_SUB"];
    else return questions["C3_Q3"];
  }

  if (currentStepId === "C3_Q2_SUB") {
    if (answerIndex === 0) return toCommon("단순 타박상");
    else if (answerIndex === 1) return toCommon("외상성 궤양");
