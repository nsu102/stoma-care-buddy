...
export const QUESTIONS: Record<string, { text: string; options: string[] }> = {
  ...
  E_Q4: { text: "장루가 길게 튀어나와 색이 변하고 아프거나 피부 사이가 깊게 벌어졌나요?", options: ["예", "아니요"] },

  // 클래스 1: 정상/창백함
  C1_Q1: { text: "장루와 주변 피부 색깔이 어떤가요?", options: ["장루가 하얗게 변함", "피부만 하얗게 붊", "선홍색 정상"] },
  ...
  // 공통 마무리 질문
  COMMON_Q1: { text: "[마지막] 장루의 모양이나 높이가 평소와 다른가요?", options: ["길게 튀어나옴(탈출)", "안으로 파고듦(함몰)", "기침 시 솟음(탈장)", "평소와 같음"] }
};
